...
$$
  min \ \  v^o + k \cdot v^{tc}
$$
A suitable value for $k$ could be calculated by dividing the number of training days with the number of holding period days increased with factor $2.5$. This can roughly be interpreted that the transaction costs error is weighted as a $2.5$ days error. For example a 4 months training period with 96 workingdays and a holdiong period of 1 month with 24 workingdays prduces the following value:
$$
...
rebalance_const <- function(old_wgt, new_wgt, old_shares, new_shares, nav, first_date = F, expo=1, x, returns){
  ...
  return(reblance_fit + 10 * tc_return^2)
}
