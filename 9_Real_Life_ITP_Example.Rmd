...
#save(QP_perf, QP_infos, QP_tickers, QP_solutions, file="analyses/ITP_long_QP_v5.rdata")
...
rebalance_const <- function(old_wgt, new_wgt, old_shares, new_shares, nav, first_date = F, expo=1, x, returns){
  ...
  tc_return <- (1+ 1 * sum(((new_shares-old_shares)!=0))/nav)^(2/expo)-1
  tc_fit <- t(rep(tc_return, nrow(returns))) %*% returns %*% x
  return(reblance_fit + tc_fit)
}
...
for(i in (train_months+1):(length(rebalance_dates)-1)){
  ...
  for(k in 1:20){
    if( k == 1 || res_pso_SAvel_MSTE$const_break > 0){
      ...
      res_pso_SAvel_MSTE$fitness <- sqrt(calc_fit(res_pso_SAvel_MSTE$solution))/nrow(returns)
...
#save(MSTE_perf, MSTE_infos, MSTE_nav, file="analyses/ITP_long_MSTE_v5.rdata")
...
#save.image("analyses/ITP_praxis_v5.rdata")
