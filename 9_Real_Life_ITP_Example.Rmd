...
# 
# #rebalance_dates <- rebalance_dates[1:50]
# 
# calc_fit <- function(x){
...
# 
# 
# rebalance_const <- function(old_wgt, new_wgt, old_shares, new_shares, nav){
#   reblance_wgt <- if(i == (train_months+1)){
#     0
#   }else{
#     sum(abs(old_wgt-new_wgt))/2
#   }
#   reblance_fit <- max(reblance_wgt-0.2,0)*5
#   reblance_fit <- reblance_fit + 1 * sum(((new_shares-old_shares)!=0))/nav
#   return(reblance_fit)
# }
# 
# 
...
#   bm_returns <- bm_returns_raw[train_frame,]
#   
#   dec_intensity <- 0.994^((nrow(returns)-1):0)
#   
#   mat <- list(
#     Dmat = t(returns*dec_intensity) %*% (returns*dec_intensity),
#     dvec = t(returns*dec_intensity) %*% (bm_returns*dec_intensity),
#     Amat = t(rbind(
#       -rep(1, ncol(returns)), # sum w <= 1
#       rep(1, ncol(returns)), # sum w >= 0.99
#       diag(1, 
...
# 
# 
# #save.image("analyses/ITP_praxis_v2.rdata")
...
for(i in (train_months+1):(length(rebalance_dates)-1)){
  ...
  for(k in c(length(ticker_temp), seq(from=length(ticker_temp)-length(ticker_temp)%%10, to = 100, by=-10))){
...
# for(i in 7:80){
#   print(paste0(sum(!QP_tickers[[i+1]] %in% QP_tickers[[i]]), "  ", length(unique(QP_tickers[[i+1]]))))
# }


#save(QP_perf, QP_infos, QP_tickers, file="analyses/ITP_long_QP_v4.rdata")


rebalance_const <- function(old_wgt, new_wgt, old_shares, new_shares, nav, first_date = F){
  reblance_wgt <- if(first_date){
    0
  }else{
    sum(abs(old_wgt-new_wgt))/2
  }
  reblance_fit <- max(reblance_wgt-0.2,0)*100
  reblance_fit <- reblance_fit + (1+ 1 * sum(((new_shares-old_shares)!=0))/nav)^(1/24)-1
  return(reblance_fit)
}
...
for(i in (train_months+1):(length(rebalance_dates)-1)){
  ...
  dec_intensity <- 1^((nrow(returns)-1):0)
  ...
  for(k in 1:20){
    if( k == 1 || res_pso_SAvel_MSTE$const_break > 0){
      res_pso_SAvel_MSTE <- pso_self_adaptive_velocity(
...
#save(MSTE_perf, MSTE_infos, MSTE_nav, file="analyses/ITP_long_MSTE_v4.rdata")
...
for(i in (train_months+1):(length(rebalance_dates)-1)){
  ...
  for(k in 1:20){
    if( k == 1 || res_pso_SAvel_CRT$const_break > 0){
      ...
      res_pso_SAvel_CRT$solution <- as.vector(round(res_pso_SAvel_CRT$solution*as.numeric(last(CRT_nav))/prices)*prices/as.numeric(last(CRT_nav)))
      res_pso_SAvel_CRT$perf <- calc_portfolio_returns(pool_data$returns[test_frame, colnames(returns)], res_pso_SAvel_CRT$solution)
      #res_pso_SAvel_CRT$fitness <- calc_fit(res_pso_SAvel_CRT$solution)
      res_pso_SAvel_CRT$const_break <- calc_const(res_pso_SAvel_CRT$solution)
...
#save(CRT_perf, CRT_infos, CRT_nav, file="analyses/ITP_long_CRT_v4.rdata")
...
#save.image("analyses/ITP_praxis_v3.rdata")
plotly_line_chart_xts(ret_to_cumret(perf_all))
