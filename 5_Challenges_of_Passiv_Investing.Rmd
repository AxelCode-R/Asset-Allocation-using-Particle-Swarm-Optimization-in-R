...
---
# Challenges of Passive Portfolio Management {#challenges}
...
### MVP: Problem
Let there be $N$ assets and their returns on $T$ different days, creating a return matrix $R \in \mathbb{R}^{T \times N}$. Each element $R_{t,i}$ contains the return of the $i$-th asset on day $t$. The estimated covariance matrix of the returns is $\textstyle\sum \in \mathbb{R}^{N \times N}$ and the estimation of the expected returns are $\mu \in \mathbb{R}^{N}$. The MVP problem with the risk aversion parameter $\lambda \in [0,1]$, as shown in [@Mari2005], can be formalized as follows:
...
```{r MVPex1, include = knitr::is_html_output(), class.source="code_fold_it_collapsed"}
returns <- buffer(
...
```{r MVPex2, echo = F, fig.cap = "Cumulative daily returns of IBM, Apple and Google"}
plotly_line_chart_xts(ret_to_cumret(returns)) %>% 
  html_save(., expand = c(-20,0,-15,0), vheight=350, vwidth=700)
...
```{r MVPex3, results = "hold", echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", size_out = "fontsize{11}{12}\\selectfont"}
...
```{r MVPex4, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed"}
...
```{r MVPex5, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed", warning=F, message=F, fig.cap = "Optimal MVP for each lambda and interpolated efficiency frontier"}
plot_ly(data = mu_and_var, type = 'scatter', mode="lines+markers") %>%
  ...
  #config(mathjax = 'cdn') %>% 
  html_save(., expand = c(-23,0,-5,0), vheight=350, vwidth=700)
```
...
```{r MVPex6, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed", cache=F, warning=F, message=F, fig.cap="Portfolio composition of MVP\'s in respect of each lambda"}
p <- plot_ly(type="bar") %>% 
...
p %>% 
  config(mathjax = 'cdn') %>% 
  html_save(., expand = c(-23,0,-11,0), vheight=350, vwidth=700)
...
```{r, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap = "Comparison of the use of the arithmetic and geometric means in the calculation of MVP\'s for each lambda. The plot of MVP\'s in terms of mu - sigma is calculated with the arithmetic mean using the resulting portfolio weights"}
...
plot_ly(data = all, type = 'scatter', mode="lines+markers", y = ~mu, x = ~sd, name=~type) %>%
  ...
  config(mathjax = 'cdn') %>% 
  html_save(., expand = c(-23,0,-7,0), vheight=350, vwidth=700)
```
...
### ITP with MSTE objective (ITP-MSTE) {#ITPMSTE}
A good explanation of the ITP with MSTE objective can be found in [@AhBa2017]. The objective is to minimize the mean square tracking error (MSTE) of daily portfolio returns $r_{t, p}$ and daily benchmark returns $r_{t, bm}$ on $T$ historical days:
...
```{r ITPex, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap = "Comparison of ITP-TEV and ITP-MSTE objectives, presented as cumulative daily returns"}
pool_returns <- buffer(
  get_yf(tickers = c("IBM", "GOOG", "AAPL"), from = "2018-01-01", to = "2019-12-31")$returns, 
  "CPI_3_assets"
)
...
```{r ITPex2, echo=F, size_out = "fontsize{11}{12}\\selectfont"}
