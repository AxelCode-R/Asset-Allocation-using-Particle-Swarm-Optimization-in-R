...
### ITP with MSE objective
A good explanation of the ITP with MSE objective can be found in [ahmedbadary](https://ahmedbadary.github.io/work_files/research/conv_opt/hw/iftp). The objective is to minimize the mean square tracking error of the daily portfolio returns $r_{p, t}$ and daily benchmark returns $r_{b,t}$ on $T$ days:

$$
  \frac{1}{T} \sum^T_{t=1}(r_{t, p}-r_{t, b})^2
$$
The formula can be rewritten as vector norm:
$$
  \frac{1}{T} \left\Vert r_{p}-r_{b} \right\Vert_2^2
$$
Which results in the following minimization with neglected stretching factor:
$$
 min \ \  \left\Vert r_{p}-r_{b} \right\Vert_2^2
$$
...
$$
  \left\Vert r_{p}-r_{b} \right\Vert_2^2 
  = \left\Vert R \times w-r_{b} \right\Vert_2^2 
  = (R \times w-r_{b})^T \times (R \times w-r_{b}) 
  = (w^T \times R^T-r_{b}^T) \times (R \times w-r_{b}) 
  = w^T \times R^T \times R \times w - w^T \times A^T \times r_b - r_b^T \times R \times w + r_b^T \times r_b
$$
The minimization of this problem and the fact that the scalars $w^T \times A^T \times r_b$ and $r_b^T \times R \times w$ are equal, transforms it to:
$$
  \min\limits_{w} \ \  \left\Vert r_{p}-r_{b} \right\Vert_2^2 
  = w^T \times R^T \times R \times w - 2\cdot r_b^T \times R \times w
$$
This leads to the equivalent general representation of the ITP as follows:
$$
