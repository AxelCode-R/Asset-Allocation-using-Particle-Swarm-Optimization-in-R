...
plotly_line_chart_xts(ret_to_cumret(returns)) %>% 
  html_save(., expand = c(-20,0,-15,0), vheight=300, vwidth=700)
...
```{r MVPex3, results = "hold", echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", size_out = "fontsize{10}{10}\\selectfont"}
...
plot_ly(data = mu_and_var, type = 'scatter', mode="lines+markers") %>%
  ...
  #config(mathjax = 'cdn') %>% 
  html_save(., expand = c(-23,0,-5,0), force=T, vheight=300, vwidth=700)
```
...
p %>% 
  config(mathjax = 'cdn') %>% 
  html_save(., expand = c(-23,0,-11,0), force=T, vheight=300, vwidth=700)
...
```{r ITPex2, echo=F, size_out = "fontsize{10}{10}\\selectfont"}
