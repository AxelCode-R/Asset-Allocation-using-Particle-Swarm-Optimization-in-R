...
plot_ly(data = mu_and_var) %>%
  ...
  layout(
    yaxis = list(range=c(min(mu_and_var$mu)*0.9, max(mu_and_var$mu)*1.1), title=plotly::TeX("\\mu")),
    xaxis = list(range=c(min(mu_and_var$sd)*0.95, max(mu_and_var$sd)*1.05), title=plotly::TeX("\\sigma"))
    # margin = list(
    #     l = 20,
    #     r = 10,
    #     b = 70,
...
p %>% config(mathjax = 'cdn') %>% html_save()
