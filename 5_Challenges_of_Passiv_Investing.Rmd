...
```{r MVP_ex2, echo = F, fig.cap = "Cumulative daily returns of IBM (IBM), Apple (AAPL) and Google (GOOG)"}
...
```{r MVP_ex5, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed", warning=F, message=F, fig.cap = "Optimal MVP for each $\\lambda$ (lambda) and interpolated efficiency frontier"}
...
```{r MVP_ex6, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed", cache=F, warning=F, message=F, fig.cap="Portfolio composition regarding each $\\lambda$ (lambda)"}
p <- plot_ly(type="bar") %>% 
...
```{r, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap = "Comparison of the use of the arithmetic and geometric means in calculating MVP\'s for each lambda. Plot of MVP\'s in terms of $\\mu$-$\\sigma$ (mu - sigma) calculated using only the arithmetic mean."}
...
```{r ITP_ex, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap = "Comparison of ITP-TEV and ITP-MSTE objectives, presented as cumulative daily returns."}
pool_returns <- buffer(
  get_yf(tickers = c("IBM", "GOOG", "AAPL"), from = "2018-01-01", to = "2019-12-31")$returns, 
  "CPI_3_assets"
)
