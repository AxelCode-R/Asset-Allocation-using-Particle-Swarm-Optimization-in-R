...
```{r MVPex6, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed", cache=F, warning=F, message=F, fig.cap="Portfolio composition regarding each lambda"}
p <- plot_ly(type="bar") %>% 
...
```{r, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap = "Comparison of the use of the arithmetic and geometric means in calculating MVPs for each lambda. Plot of MVPs in terms of mu - sigma calculated using only the arithmetic mean."}
