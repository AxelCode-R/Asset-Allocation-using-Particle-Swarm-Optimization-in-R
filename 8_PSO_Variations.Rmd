...
df_res <- df %>% group_by(type, iter) %>% summarise(time=mean(time), mean_fit = mean(mean_fit), best_fit = mean(best_fit)) %>% ungroup()

plot_ly(data = df_res, x=~iter, y=~best_fit, name = ~paste0("best_fit_", type), mode="lines", type = 'scatter')
