...
```{r variants1, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap = "Fitness: Benchmark solution with the standard PSO for comparison with the created variants."}
nav <- 20000
...
plot_ly() %>% 
  ...
  layout(yaxis=list(range=c(min(df_res[df_res$iter>30,]$best_fit_q1)-0.0005,  max(df_res[df_res$iter>30,]$best_fit_q3)+0.0005), title="best fitness")) %>% 
  html_save()
```
...
```{r variants2, echo=F, fig.cap = "Statistics: Benchmark solution with the standard PSO for comparison with the created variants."}
reactable(
  df_res[df_res$iter==max(df_res$iter),],
  defaultColDef = colDef(align = "left"),
  columns = list(
    iter = colDef(width=50),
    type = colDef(width=90),
    time_mean = colDef(format = colFormat(digits=2), width=90),
    const_break_mean = colDef(format = colFormat(digits=6), width=145),
    best_fit_q1 = colDef(format = colFormat(digits=6), width=100),
    best_fit_q3 = colDef(format = colFormat(digits=6), width=100),
    best_fit_mean = colDef(format = colFormat(digits=6), width=110),
    best_fit_median = colDef(format = colFormat(digits=6), width=125)
  ),
  compact = T,
  rownames = F,
  style = list(fontFamily="Computer Modern", fontSize=13)
) %>% 
  html_save(., vwidth = 870, expand = c(-40,-20,-60,-20))
```


## Function Stretching
PSO often gets stuck in local minima, i.e. when the current global best position is a local minimum with a surrounding region, with only higher fitnesses. In such situations, it is difficult for the PSO to escape to find the global minimum. Function stretching attempts to allow the PSO to escape from such local minima by transforming the fitness function in the manner described in [@PaVr2002]. After finding a local minimum, a two-stage transformation proposed by Vrahatis in 1996 can be used to stretch the original function such that the discovered local minimum is transformed into a maximum, but any position with less fitness remains unchanged. The two stages of the transformation with a discovered local minimum $\bar{x}$ are:
\begin{equation} 
...
The function $G(\bar{x})$ can be simplified to $f(\bar{x})$, the norm used in $G(x)$ is the Manhattan norm and the $\text{sign}()$ function is defined as follows:
$$
  \text{sign}(x) = 
  \begin{cases}
    1, & \text{if}\ \ x > 0\\
...
```
and the domain of definition is chosen as $x \in [-20, 20]$. Suppose the PSO gets stuck in the local minimum at $\bar{x} = \pi - \text{arcsin}(\frac{1}{10}) \approx 3.04$. The original function `fn` and the transformed function `fn_stretched`, which matches $H(x)$ in equation \@ref(eq:fnstretchH), are shown in the following graph:


```{r, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap="Analysis of the effect of function stretching using a simple 1D example."}
...
```{r variants3, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap="Comparison of fitness between the PSO with functional stretching and the standard PSO."}
set.seed(0)
...
![Visualization of the simple neighborhood topology.](img/PSO_local_chart.jpg)
...
```{r variants5, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap="Comparison of fitness between the local PSO and the standard PSO."}
set.seed(0)
...
```{r variants6next, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap="Comparison of fitness between the PSO with feasibility preservation and the standard PSO, starting from the same randomly selected initial particles obtained from the last iterations of the standard PSO benchmark solutions."}
set.seed(0)
...
```{r variants6next2, echo=F, fig.cap = "Comparison of statistics between the PSO with feasibility preservation and the standard PSO, starting from the same randomly selected initial particles obtained from the last iterations of the standard PSO benchmark solutions."}
reactable(
  df_res[df_res$iter==max(df_res$iter),],
  defaultColDef = colDef(align = "left"),
  columns = list(
...
```{r variants7, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap="Comparison of fitness between the PSO with self-adaptive velocity and the standard PSO."}
set.seed(0)
...
```{r variantspkg, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap="Comparison of fitness between the PSO R-package with standard PSO settings and the standard PSO."}
set.seed(0)
