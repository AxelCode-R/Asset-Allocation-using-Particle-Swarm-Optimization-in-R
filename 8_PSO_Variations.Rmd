...
Update the velocity of each particle $d$ with the following switch-case for a uniform random number $r \in \text{Unif}(0,1)$ in iteration $i+1$:
$$
  v_d^{i+1} = 
  \begin{cases}
    w_d^i \cdot v_d^{i}+c_{p,d}^i \cdot \text{Unif}(0,1) \cdot (P_{d}^i-x_d^i) + c_{g,d}^i \cdot \text{Unif}(0,1) \cdot (p_{g}^i-x_d^i), & \text{if}\ \ r > 0.8\\
    w_d^i \cdot v_d^{i}+c_{p,d}^i \cdot \text{Cauchy}(\mu_1, \sigma_1) \cdot (P_{d}^i-x_d^i) + c_{g,d}^i \cdot \text{Cauchy}(\mu_1, \sigma_1) \cdot (p_{g}^i-x_d^i), & \text{if}\ \ 0.8 \geq r > 0.4\\
    w_d^i \cdot v_d^{i}+c_{p,d}^i \cdot \text{Cauchy}(\mu_2, \sigma_2) \cdot (P_{d}^i-x_d^i) + c_{g,d}^i \cdot \text{Cauchy}(\mu_2, \sigma_2) \cdot (p_{g}^i-x_d^i), & \text{if}\ \ 0.4 \geq r\\
  \end{cases}
$$
with 
\begin{align*}
...
$$
  ...
  \end{cases}
$$
3) Self-Adaptive Control Parameters
...
5) Repeat
Step 2 to 4 are repeated until the maximal iterations $i_{max}$ is reached.
...
p1 <- plot_ly() %>% 
  layout(xaxis=list(range=c(-2, 3)), yaxis=list(range=c(-0.1,3.3)))
