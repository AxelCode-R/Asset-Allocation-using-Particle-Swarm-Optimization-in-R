...
```{r variants1, eval = T, include = knitr::is_html_output(), echo= knitr::is_html_output(), class.source="code_fold_it_collapsed"}
nav <- 20000
...
save(df_res, df_SPSO, nav, from, to, prices, calc_fit, calc_const, mat, pool_returns, bm_returns, SPSO_feasable_solutions, file="data/save_variant1.rdata")

```


```{r variants1s, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap = "Fitness: Benchmark solution with the standard PSO for comparison with the created variants."}
...
plot_ly() %>% 
  ...
  html_save(., vheight=400, vwidth = 850, expand=c(-20, 0, 0, 0))
```
...
reactable(
  ...
  html_save(., vwidth = 808, expand = c(-45,-35,-60,-35))
```
...
```{r variants3, eval = T, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed"}
set.seed(0)
...
save(df_res, file="data/save_variant2.rdata")
```
...
```{r variants5, eval = T, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed"}
set.seed(0)
...
save(df_res, file="data/save_variant3.rdata")
```
...
save(df_res, file="data/save_variant4.rdata")
```


```{r variants6nexts, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap="Comparison of fitness between the PSO with feasibility preservation and the standard PSO, starting from the same randomly selected initial particles obtained from the last iterations of the standard PSO solutions."}
...
```{r variants7, eval = T, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed"}
set.seed(0)
...
save(df_res, file="data/save_variant5.rdata")
```
...
```{r variantspkg, eval = T, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed"}
set.seed(0)
...
save(df_res, file="data/save_variant6.rdata")
```
