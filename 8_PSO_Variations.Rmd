...
# })
# 
# df <- suppressWarnings(rbind(data.frame(
#     "type" = "PSO", "time"=res_pso_time[3], res_pso$fit_data %>% select(iter, "mean_fit"=mean, "best_fit"=best)
#   ),
#   data.frame(
...
df_res <- df %>% 
  ...
  group_by(iter, type) %>% 
  summarise(time=mean(time), best_fit_q1 = quantile(best_fit, 0.05), best_fit_q3 = quantile(best_fit, 0.95), mean_fit = mean(mean_fit), best_fit_median = quantile(best_fit, 0.5)) %>% 
  ungroup()

# plot_ly(data = df_res, x=~iter, y=~best_fit_agg, name = ~paste0("best_fit_", type), mode="lines", type = 'scatter') %>% 
#   add_trace(data = df_res, x=~iter, y=~best_fit_q1, name = ~paste0("best_fit_", type)) %>% 
#   add_trace(data = df_res, x=~iter, y=~best_fit_q3, name = ~paste0("best_fit_", type), fill="tonexty") %>% 
#   layout(yaxis=list(range=c(min(df_res$best_fit_agg)-0.01, mean(df_res[df_res$iter>5,]$best_fit_agg)+0.01)))
...
plot_ly() %>% 
  ...
  add_trace(data = df_res %>% filter(type=="PSO"), x=~iter, y=~best_fit_q1, name = "PSO_q1", mode="lines", type = 'scatter', line = list(color="rgba(255, 51, 0, 0.4)")) %>% 
  add_trace(data = df_res %>% filter(type=="PSO"), x=~iter, y=~best_fit_q3, name = "PSO_q3", mode="lines", type = 'scatter', fill="tonexty", line = list(color="rgba(255, 51, 0, 0.4)"), fillcolor = "rgba(255, 51, 0, 0.4)") %>% 
  ...
  add_trace(data = df_res %>% filter(type=="PSO-fnS"), x=~iter, y=~best_fit_q1, name = "PSO-fnS_q1", mode="lines", type = 'scatter', line = list(color="rgba(0, 102, 204, 0.3)")) %>% 
  add_trace(data = df_res %>% filter(type=="PSO-fnS"), x=~iter, y=~best_fit_q3, name = "PSO-fnS_q3", mode="lines", type = 'scatter', fill="tonexty", line = list(color="rgba(0, 102, 204, 0.3)"), fillcolor = "rgba(0, 102, 204, 0.3)") %>% 
