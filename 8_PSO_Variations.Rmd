...
```{r variants1s, echo= knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap = "Fitness: Benchmark solution with the standard PSO for comparison with the created variants.", strip.white=T}
...
```{r variants2, echo=F, fig.cap = "Statistics: Benchmark solution with the standard PSO for comparison with the created variants.", strip.white=T}
reactable(
...
subplot(p1, p2, shareY=T, nrows=2, margin=0.05) %>% 
  html_save(., vheight=500, vwidth=800)
```
...
plot_ly(x=X, y=sapply(X, fn1, pos_best=x_best, pos_best_fit=x_best_fit), type="scatter", mode="lines", name="fn_stretched", line=list(color="orange")) %>% 
  ...
  html_save(., vheight=225, vwidth=800)
```
