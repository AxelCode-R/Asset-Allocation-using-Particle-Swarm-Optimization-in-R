...
### Daily Returns
The default timeframe for all raw data in this thesis is one workday and only simple returns are used. Suppose there is a asset with prices $P$ on workday $t_i$ and $t_{i+1}$, then follows that the simple return on $t_{i+1}$ can be calculated as:
$$
...
### Implicit Daily Returns {#implicitdailyreturns}
...
# compound
# erklären von
# geometric average return (is used)
# arithmetic average return

R1 <- rnorm(n = 100000, mean = 0, sd = 0.001)

p1 <- plot_ly(mode="lines", type = 'scatter') %>% 
  add_trace(x=1:length(R1), y = cumprod(1+R1), name="correct", line=list(color="grey")) %>% 
  add_trace(x=1:length(R1), y = cumprod(rep(1+mean(R1), length(R1))), name="mean returns", line=list(color="green")) %>% 
  add_trace(x=1:length(R1), y = cumprod(rep(prod(1+R1)^(1/length(R1)), length(R1))), name="implicit returns", line=list(color="blue"))


R2 <- rev(-R1)
p2 <- plot_ly(mode="lines", type = 'scatter') %>% 
  add_trace(x=1:length(R2), y = cumprod(1+R2), name="correct", showlegend=F, line=list(color="grey")) %>% 
  add_trace(x=1:length(R2), y = cumprod(rep(1+mean(R2), length(R2))), name="mean returns", showlegend=F, line=list(color="green")) %>% 
  add_trace(x=1:length(R2), y = cumprod(rep(prod(1+R2)^(1/length(R2)), length(R2))), name="implicit returns", showlegend=F, line=list(color="blue"))
