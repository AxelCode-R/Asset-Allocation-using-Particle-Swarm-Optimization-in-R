...
PSO often gets stuck in local minimas, that means if the current global best position is a local minima with a larger neighborhood around it containing higher fitness, it's hard for the PSO to escape and find the global minima. Function stretching tries to accomplish that the PSO can escape from such local minimas, by transforming the fitness function in a manner that is described in \citep{PaVr2002}. It states that after a local minima is found a two stage transformation proposed by Vrahatis 1996 can be used to stretch the original function in a way that the detected local minima transforms into a maxima, but any position with less fitness will stay unchanged. The two stages of the transformation with a detected local minima \(\bar{x}\) are:
\[
...
The PSO with particle neighborhoods is called \texttt{PSO-local} and is evaluated on the test problem with \(k=20\):
...
\item
  ...
  \begin{align*}
    ...
    \sigma_2 &= 0.4
  \end{align*}
  and \(\text{Cauchy}(\mu, \sigma)\) is a random number generated from the Cauchy distribution, that can be obtained with \texttt{rcauchy()} in R. The position update is the same as in the standard PSO. If a particle \(d\) left the feasible search space in it's coordinate \(z\), it gets pushed back with the following switch-case for \(r = \text{Unif}(0,1)\):
  \[
    x_{d,z} = 
    \begin{cases}
...
\item
  ...
  For a objective function \(f()\) and the maximal fitness of all particles \(f_{max} = max(f(X^{i+1}))\), the parameters \(w_d^{i}\), \(c_{p,d}^{i}\) and \(c_{g,d}^{i}\) for each particle \(d\) are adjusted as follows:
  \begin{align*}
...
The PSO with self-adaptive velocity is called \texttt{PSO-SAvel} and is evaluated on the test problem with the constants used in the implementation section:
