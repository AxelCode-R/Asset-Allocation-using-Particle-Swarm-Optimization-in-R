...
The PSO was developed by J. Kennedy as a global optimization method based on swarm intelligence and presented to the public in 1995 by Eberhart and Kennedy [@KeEb1995]. The original PSO was intended to resemble a flock of birds flying through the sky without collisions. Therefore, its first applications were found in particle physics to analyze moving particles in high-dimensional spaces, which the name Particle recalls. Later, it was adapted in Evolutionary Computation to exploit a set of potential solutions in high dimensions and to find the optima by cooperating with other particles in the swarm [@PaVr2002]. Since it does not require gradient information, it is easier to apply than other global optimization methods. It can find the optimum by considering only the result of the function to be optimized. This means that the function can be arbitrarily complex and it is still possible to reach the global optimum. Other advantages are extensibility, simplicity, and low computational cost, since only basic mathematical operators are used.
...
```{r gifanimate, echo=F, eval=F, include=F}
...
grid <- setNames(expand.grid(seq(-10, 10, length.out = 100), seq(-10, 10, length.out = 100)), c("x1", "x2"))

grid$fitness <- apply(grid, 1, f)

grid <- grid %>% spread(., key = x1, value = fitness) %>% column_to_rownames("x2") %>% as.matrix()
...
plot_ly(z = grid, type = "contour", y = as.numeric(rownames(grid)), x = as.numeric(colnames(grid)), showscale = F) %>%
  ...
  html_save(., force=T, zoom = 1, vheight = NULL, vwidth = NULL, selector = ".cartesianlayer")

image_file <- "docs/p.png"
...
fig <- plot_ly() %>%
      add_trace(
        ...
        type = 'scatter',
        showlegend=F,
        marker = list(color = 'red', size=9, opacity = 1, showscale = F)
      ) %>%
      animation_opts(redraw=F, easing="cubic-in-out", transition = 500, frame = 700) %>%
      layout(
        xaxis = list(title = "axis_1", gridcolor = '#0000', zerolinewidth = 0, 
                     zeroline = FALSE, zerolinecolor = '#0000', range=c(min(as.numeric(colnames(grid))), max(as.numeric(colnames(grid))))),
        yaxis = list(title = "axis_2", gridcolor = '#0000', zerolinewidth = 0, 
                     zeroline = FALSE, zerolinecolor = '#0000', range=c(min(as.numeric(rownames(grid))), max(as.numeric(rownames(grid))))),
        images = list(
          list(
            # Add images
...
```{r gifanimate2, echo=F, eval=F, include=F}
...
trace_data <- res$trace_data %>% mutate(iter = iter+10) 
for(i in 1:10){
  trace_data <- rbind(trace_data, trace_data %>% filter(iter==11) %>% mutate(iter=i))
}
trace_data <- trace_data %>% arrange(iter)

temp <- sapply(list.files('gifs/pso_2dim_v2/', full.names = T), file.remove)
for(i in 1:length(unique(trace_data$iter)) ){
  plot_ly() %>%
      add_trace(
        data=trace_data[trace_data$iter==i,],
        x=~X1,
        y=~X2,
        name = "",
        mode ='markers',
      ...
      layout(
        xaxis = list(title = "axis_1", gridcolor = '#0000', zerolinewidth = 0, tickfont=list(size=18), titlefont=list(size=18), 
                     zeroline = FALSE, zerolinecolor = '#0000', range=c(min(as.numeric(colnames(grid))), max(as.numeric(colnames(grid))))),
        yaxis = list(title = "axis_2", gridcolor = '#0000', zerolinewidth = 0, tickfont=list(size=18), titlefont=list(size=18),
                     zeroline = FALSE, zerolinecolor = '#0000', range=c(min(as.numeric(rownames(grid))), max(as.numeric(rownames(grid))))),
        images = list(
          list(
            # Add images
      ...
      config(displayModeBar = FALSE) %>% 
      html_save(., force=T, vheight = 600, vwidth = 800, delay = 1)
  file.copy(from = "docs/p.png", to = paste0("gifs/pso_2dim_v2/gif_2dim_", i,".png"))
}
...
First, we show whether the PSO satisfies the *algorithm condition* and the *convergence condition (for local search)* to be classified as a local search algorithm, i.e., that the PSO is guaranteed to converge to at least a local minimum. Formally, this is defined in the following theorem:
...
The optimality region $R_{l,\epsilon}$ in the above theorem is defined as
...
The analysis starts with the *algorithm condition*, which is defined as follows:
...
The region $R_{g,\epsilon}$ in the above theorem can be interpreted as the arbitrarily small bounded subset of $S$ containing the global minimum. Moreover, the *convergence condition (for global search)* is defined as follows:
...
This section summarizes the mathematical convergence analysis of the standard PSO with constant inertia weight $w$ performed in [@FbEn2010] and in [@FbEn2007], whose main goal is to prove whether the PSO converges to a local or even a global minimum. Results from the sources are presented in a simplified form so that the full scope can be understood. For this purpose, mathematical principles are not proven, otherwise it would be beyond the scope of this thesis. It is recommended to look at the detailed proofs in the mentioned sources to gain a deeper understanding. This section first begins by gathering the conditions that must be satisfied for a general stochastic search algorithm to be classified as a local or global search algorithm. Then, these conditions are used to classify the standard PSO with constant inertia weight $w$. It follows that certain conditions must be imposed on the hyperparameters and the PSO must be modified to be classified as a global or local search algorithm. Finally, these findings are applied to the practical implementation of the PSO in order to draw relevant conclusions for practical applications.

### Convergence of a General Stochastic Search Algorithm
...
Next, we need to change the definition of the global minimum. Since the probability of generating an exact point in a search space $S$ with a stochastic algorithm is zero, we define an optimality region $R_{\epsilon}$ that has a nonzero volume:
...
This means that it is sufficient to show that the generation of new positions in the algorithm have probability zero to miss any subset in $S$ with a positive volume infinitely often. For example, an algorithm that generates the new position uniformly distributed on $S$ would satisfy this condition.
...
Now we address the local convergence of the algorithm, which in addition to the *algorithm condition* must also satisfy the following condition:
...
First of all it has to be ensured that the PSO satisfies the *algorithm condition*. That is, the *algorithm condition* is satisfied if the algorithm has a mapping $D$ that compares two positions and returns the position with the lowest objective value that is also in the domain of definition. In terms of the PSO, we are concerned with the behavior when the global best position is changed. If a new personal best position $\xi$ is found, it is guaranteed to be in the definition space, since all calculated positions are pushed back to their edge if they are outside the definition space. Then, the PSO compares the objective value of $\xi$ with the objective value of the previous global best position $z$ and stores the position with the lowest objective value as the global best position. Thus, the PSO satisfies the *algorithm condition*. 
...
Returning to the convergence analysis of the PSO analyzed in [@FbEn2010], which uses parameters $w$, $c_p$, and $c_g$ that lie in the convergent parameter region. This leads to a standard PSO that converges with all particles to a single point $\hat{p}$.

Accordingly, the *convergence condition (for local search)* must be satisfied, which means that the PSO must be able to reduce the distance to the optimality region in each iteration with a downward bounded non-zero probability. Unfortunately, this is not the case, as can be seen in a simple example. Suppose the PSO is to minimize a 2-dimensional objective function $f$ with only two particles and the initial positions are $x_1=(k_1, k_2)$, $x_2=(k_1, k_2) + a_1 \cdot (1, 0)$ and the initial velocities are $v_1=a_2 \cdot (1, 0)$ and $v_2=a_3 \cdot (1, 0)$ with constants $k_1, k_2, a_1, a_2$, and $a_3$. It follows that all particles and saved positions have $k_2$ in the second coordinate, resulting in a zero in the second coordinate of the velocity update in all iterations. Therefore, the second coordinate with $k_2$ is not guaranteed to lead to the optimality region and the PSO cannot improve in this coordinate, which contradicts with the *convergence condition (for local search)* and it follows that the PSO is not a local search algorithm.

The possibility of this premature stagnation in a coordinate decreases significantly with increasing particle number, which is why it is very unlikely in practice. In order for the PSO to still be classified as a local search algorithm, a variant of the PSO was developed in [@FbEn2010], called the guaranteed convergence PSO (GCPSO). In this variant, it is guaranteed that it is at least possible to randomly shift the position of the global best particle in each coordinate by a uniformly generated value from the interval $[-\rho_i, \rho_i]$ in iteration $i$. Therefore, the GCPSO is classified as a local search algorithm. The exact implementation of the value $\rho_i$ and how it is changed per iteration is quite technical and is not exactly reproduced here. Further, it can be imagined that $\rho_0$ is chosen large enough and doubles per iteration if the global best position is successfully changed and halves if the global best particle is not changed. This behavior of the GCPSO is needed to argue why the space of functions is larger than the unimodal functions. Suppose the function $f$ on $S$ 




Angenommen die Funktion $f$ ist nicht unimodal und besitzt demnach mehrere lokale Minimas auf $S$. Es ist aber möglich den Raum $S$ und alle Minimas $k$ in kompakte Teilgebiete $\hat{S}_k \subseteq S$ mit $\hat{S}_j \cap \hat{S}_t = \emptyset \ \forall j\neq t$ aufzuteilen auf denen $f$ wiederum unimodal ist und somit genau ein Minimum besitzt. Daraus folgt das wenn man das GCPSO mit Hyperparametern aus der konvergenten parameterregion wählt und auf dieses Problem angewendet wird, es anfangs zwischen den Teilgebieten $\hat{S}_k$ herumspringt. Auf grund der wahl der Parameter wird sich der Schwarm aber irgendwann zusammenziehen und in richtung des global besten Partikels konvergieren. Ab diesem zeitpunkt stagniert das GCPSO weshalb das $\rho_i$ immer kleiner wird. Ab einer gewissen Iteration ist es dem GCPSO nicht mehr möglich das Teilgebiet $\hat{S}_k$ zu verlassen. Anschließend gelten alle anforderungen an die *convergence condition (for local search)* weshalb der Schwarm mit wahrscheinlichkeit 1 gegen das Minimum in diesem Teilgebieten $\hat{S}_k$ konvergiert. Es kann nicht sichergestellt werden in welchem Teilgebieten $\hat{S}_k$ das passiert, weshalb nur gesagt werden kann dass das GCPSO gegen ein lokales Minimum konvergiert, bei dieser Wahl der Gruppe von Funktionen. 
...
### Convergence of a General Search Algorithm
First, we collect the knowledge and conditions from the source [@FbEn2010] for a general algorithm. We start by defining a general problem where a measurable function $f: \mathbf{R}^n\rightarrow\mathbf{R}$ has to be minimized on the search space $S$. For this purpose, a procedure is used which starts with a position from the search space and in each iteration compares its old position with a new one and keeps one of them. The comparison of the positions and the return of one of them is denoted by the mapping $D$. For the procedure to be classified as an algorithm, it is sufficient to show that the following *algorithm condition* is satisfied:

**Algorithm condition**: The mapping $D: S \times \mathbb{R}^n\rightarrow S$ should satisfy $f(D(z, \xi)) \leq f(z)$ and if $\xi \in S$, then $f(D(z, \xi)) \leq f(\xi)$.

Thus, the procedure is an algorithm if after each iteration the position is returned that has the lowest fitness value and also lies in the search space $S$.

Next, we investigate when the algorithm converges to at least a local minimum. Since the probability of generating an exact point in a search space $S$ with a stochastic algorithm is zero, an optimality region $R_{\epsilon}$ is defined, which has a non-zero volume:

$$
 R_{l,\epsilon} = \{z\in S\cap M_k\ \ \forall^{\infty} k \ | f(z) < \psi + \epsilon\}
$$

where $\psi$ denotes the essential infimum of $f$ on $S$, with $\epsilon > 0$ and $M_k$ is the region in iteration $k$ that has a non-zero probability of generating a new position for the algorithm $D$. The optimality region can be interpreted in a simplified way as the union of all arbitrarily small regions containing a local minimum. This interpretation is not mathematically correct, but it allows an easier understanding of the statements in this section. A more precise interpretation can be found in [@FbEn2010]. It is assumed that algorithm $D$ has found at least a local minimum if it has generated a position in the optimality region $R_{l,\epsilon}$. 

To ensure that the algorithm $D$ iteratively approaches at least a local minimum, the following *convergence condition (for local search)* is required:
...
with a probability measure $\mu_i$ and a distance measure $\text{dist}()$. This means that the algorithm must be able to get one step closer to the optimal region in each iteration with a non-zero probability.

If the *algorithm condition* and the *convergence condition (for local search)* are satisfied, it follows that the algorithm $D$ is a local search algorithm, which is summarized in the following theorem:
...
We further analyze when an algorithm $D$ is guaranteed to converge to a global minimum. For this purpose, the optimality region $R_{l,\epsilon}$ has to be restricted to contain only global minima. This is achieved by letting $S \subseteq M_k \ \forall k$, which is a special case of $R_{l,\epsilon}$, further denoted as $R_{g,\epsilon}$. Thus, in each iteration, the algorithm $D$ has a non-zero probability of generating a position in any region of the search space that has a non-zero volume. This behavior is specified by the following condition:
...
$$
  \prod_{i=0}^\infty (1-\mu_i(A)) = 0,
$$
where $\mu_i(A)$ is the probability of $A$ being generated by $\mu_i$ from $M_k$. This means that the *convergence condition (for global search)* is satisfied if there is no region in $S$ from which the algorithm $D$ cannot generate a point after infinitely many attempts.

Thus, for an algorithm $D$ to be classified as a global search algorithm, the *algorithm condition* and the *convergence condition (for global search)* must be satisfied, which is summarized in the following theorem:
...
### Convergence of the Standard PSO
...
reactable(
  ...
  columns = list(
    ...
    constraint = colDef(name = "constraint break", format = colFormat(digits=9), width=120),
    time = colDef(width=65)
  ),
  style = list(fontFamily="Computer Modern, sans-serif", fontSize=13),
  defaultColDef = colDef(align="left")
) %>% 
  html_save(., force=T, expand = c(-40,-20,-35,-20))
```
...
```{r pso9, echo = knitr::is_html_output(), class.source="code_fold_it_collapsed", fig.cap="Comparison of solving a discrete ITP-MSTE with PSO and the solve.QP with rounding afterwards"}
set.seed(0)
