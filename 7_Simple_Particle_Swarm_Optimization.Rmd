...
Thus, the procedure is an algorithm if after each iteration a position is returned that has the lowest fitness value and also lies in the search space $S$.
...
itp_QP <- function(pool_returns, bm_returns){
  ...
  mat <- list(
    ...
    Amat = t(rbind(
      ...
      diag(-1, 
           nrow=ncol(pool_returns), 
           ncol=ncol(pool_returns)) # w <= 0.05
    )),
    bvec = c(
      -1, # sum w <= 1
      0.99, # sum w >= 0.99
      rep(0, ncol(pool_returns)), # long only
      rep(-0.05, ncol(pool_returns)) # w <= 0.05
    ),
    meq = 0
  )
...
reactable(
  ...
  html_save(., expand = c(-40,-20,-35,-20))
```
...
A PSO approach has advantages and disadvantages, since on the one hand any problem can theoretically be solved, but it cannot be guaranteed that the solution is also optimal. In addition, the calculations take much longer than with the `solve.QP` approach, which raises the question why a PSO approach should have any benefit at all. This is exactly the case, if the solution of the problem is no longer possible by the `solve.QP` alone, as it is for example the case with mixed-integer-quadratic-problems. In this type of problems, the condition is that the variable of interest $x$ must be an integer vector, which can only be solved continuously using `solve.QP` and then rounded. However, this rounding error can become arbitrarily large, which is why the chances of the PSO approach to achieve a better solution are greater than with the `solve.QP` approach.
