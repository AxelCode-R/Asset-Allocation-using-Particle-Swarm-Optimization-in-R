...
plot_ly(
  ...
  layout(
    xaxis=list(range=c(0.9*min(df$sd), 1.5*max(df[df$type=="QP_MVP",]$sd))),
    yaxis=list(range=c(0.9*min(df$mu), 1.5*max(df[df$type=="QP_MVP",]$mu)))
  ) %>% 
  html_save()
```
...
reactable(
  ...
  columns = list(
    var = colDef(format = colFormat(digits=7)),
