...
This section summarizes the mathematical convergence analysis of the standard PSO performed in [@FbEn2010], whose main goal is to prove whether the PSO converges to a local or even a global minimum. This section is only a simplified and therefore inaccurate reproduction of the relevant results of the source without proofs, so it is recommended to also look at the exact statements and proofs in the source. First, we will show whether the PSO satisfies the *algorithm condition* and the *convergence condition (for local search)* to be classified as a local search algorithm, i.e., that the PSO is guaranteed to converge to at least a local minimum. More formally, this is defined in the following theorem:
...
$$
where $\psi$ denotes the essential infimum of $f$ on $S$, and $\epsilon > 0$. The resulting optimality region can be interpreted as a union of arbitrarily small bounded subsets of $S$ that contain a local minimum. This is needed since each point in the search space $S$ has a probability measure of zero to be randomly generated. For a precise mathematical definition of the optimality region $R_{l,\epsilon}$ see [@Fdbe2006].

Now we can start by analyzing the *algorithm condition* which is defined as follows:
...
That is, the *algorithm condition* is satisfied if the algorithm has a mapping $D$ that compares two positions and returns the position with the lowest objective value that is also in the domain of definition. In terms of the PSO, it is about the behavior regarding the change of the global best position $z := p_{g}$. If a new personal best position $\xi:=p_{p}$ is found, it is guaranteed to be in the definition region, since all computed positions are pushed back to its edge if they are outside the definition region. Afterwards, the PSO compares the objective value of $\xi$ with the objective value of $z$ and stores the position with the lowest objective value as the global best position. Thus, the PSO satisfies the *algorithm condition*. 

The next condition focuses on the convergence behavior of the PSO, which must ensure that the swarm convergences to a fixed point. In [@FbEn2010], results were referenced from [@RiPo2007], which analyzed the convergence property of a deterministic PSO, i.e., when the uniformly random numbers are replaced by their upper bounds. Afterwards, the results were verified by simulations of the stochastic standard PSO. This work was subsequently extended by [@Mjly2006] to deal directly with the stochastic version of the standard PSO. 

In [@Mjly2006] three properties are described for the behavior of the PSO. First, the trajectory of each particle $d$ is described as a sequence of random variables $\{\pmb{X}^d_i\}$, with particle positions $\pmb{X}^d_i$ in iteration $i$. Property 1 is fulfilled if the expectation of particle positions $\pmb{X}^d_i$ converges to a fixed point for each particle ($\lim_{i\rightarrow \infty} E[\pmb{X}^d_i] = p^d \ \ \forall d$). Property 2 is fulfilled if the variance of the positions tend to zero for each particle ($\lim_{i\rightarrow \infty} Var[\pmb{X}^d_i] = 0 \ \ \forall d$). Property 3 is fulfilled, if the entire particle swarm converges in mean square to a fixed point, that means all particle positions converge to a single point $\hat{p}$, which can be summarized as $\lim_{i\rightarrow \infty} E[\pmb{X}_i^d-\hat{p}]^2=0 \ \ \forall d$. 



The properties 1-3 are dependent on the choices of $c_p$, $c_g$ and $w$, which can be found in [@Mjly2006]. The proofs and deeper explanation of the constraints for parameters are beyond the scope of this thesis. In the following, the regions for each property are visualized and the blue region is further denoted as the convergent parameter region with boundaries $c_p$=$c_g$=$c>0$ and $0<w<1$:


```{r convergenceregion, echo=F, fig.cap="Relationship between $w$ and $c$ when $c_p$=$c_g$=$c$ to distinguish between different convergence behaviors. The black area shows the convergence condition for the sequence of expected positions satisfying property 1. The cyan area shows the convergence condition for the sequence of expected positions and the variances tending to zero, which satisfies properties 1 and 2. And the blue area shows the convergence condition for the sequence of expected positions with the variances tending to zero and the entire swarm converging to a single point, which satisfies properties 1, 2, and 3. This is a replication of the visualization from [M. Jiang, 2006]."}
...
with a probability measure $\mu_i$, a distance measure $\text{dist}()$.


Simple said, the *Convergence condition (for local search)* is satisfied if the PSO has a none zero probability to decrease the distance to the optimality region in every iteration. Unfortunately this is not the case which can be seen in a simple example. Suppose the PSO has to minimize a 2-dimensional objective function $f$ with only two particles and the initial positions are $x_1=(k_1, k_2)$, $x_2=(k_1, k_2) + a_1 \cdot (1, 0)$ and the initial velocitys are $v_1=a_2 \cdot (1, 0)$ and $v_2=a_3 \cdot (1, 0)$. This follows that the personal and global best positions have $k_2$ in the second coordinate, which results in a zero in the second coordinate of the velocity update in all iterations. Therefore the PSO can not improve in the second coordinate which contradicts with the *Convergence condition (for local search)* and follows that the PSO is not a local search algorithm. The possibility of a premature stagnation of improvement in one coordiante decreases rapidly with increase in the count of particles.

The possibility of this premature stagnation in a coordinate decreases considerably with increasing the number of particles, which is why it is very unlikely in practice. In order that the PSO can nevertheless be classified as a local search algorithm, a variant of the PSO was developed in [@FbEn2010], the so-called guaranteed convergence PSO (GCPSO). In this variant, it is guaranteed that if no improvement occurs, it is at least possible to shift the position of each particle in each coordinate randomly by the machine precision. Therefore the GCPSO is classified as a local search algorithm. 

To extend the GCPSO to a global search algorithm, it has to satisfy the following theorem:
...
and $R_{g,\epsilon}$ has to be interpreted as the arbitrarily small bounded subsets of $S$ that contains the global minimum. In addition the *convergence condition (for global search)* is defined as:
...
Summarized and Simple said, the algorithm is classified as a global search algorithm if the *algorithm condition* is satisfied and the algorithm has a zero probability of repeatedly missing the region $R_{g,\epsilon}$.

This behavior of the GCPSO can be achieved by farly small changes to the algorithm. For example by introducing a particle which is randomly generated every $k$ iterations. Another approach is to restart the PSO if the majority of particles are located in a small region. Further approaches and the benefits are discussed in [@FbEn2010].
...
reactable(
  ...
  style = list(fontFamily="Computer Modern", fontSize=13)
) %>% 
