...
## Example MVP
(IN PRODUKTION)
chart der pso und qp vergleicht mit lambda schritten
```{r, echo=F}
returns_raw <- buffer(
  get_yf(
...
itp_QP <- function(pool_returns, bm_returns, mat){
...
itp_PSO <- function(pool_returns, bm_returns, mat, silent = T){
...
temp_qp <- itp_QP(pool_returns_raw, bm_returns, mat)
temp_pso <- itp_PSO(pool_returns_raw, bm_returns, mat)
