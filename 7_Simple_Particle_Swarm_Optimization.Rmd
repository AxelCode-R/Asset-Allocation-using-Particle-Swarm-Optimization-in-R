...
This section summarizes the mathematical convergence analysis of the standard PSO performed in [@FbEn2010], which has the main goal to prove whether the PSO converges to a local or even a global minimum. This section provides only a simplified and therefore inaccurate reproduction of the relevant results of the source, so it is recommended to also look at the exact statements and proofs in the source. At first the PSO has to satisfy the algorithm condition for random search algorithms with the objective function $f$:
...
That is, the *algorithm condition* is satisfied if the algorithm has a mapping $D$ that compares two positions and returns the position with the lowest objective value that is also in the definition region. In relation to the PSO, this concentrates on the behavior of changing the global best position $z := p_{g}$. If a new personal best position $\xi:=p_{p}$ is found, it is guaranteed that it is in the definition region, because all positions calculated are pushed back to there boundary, if they are outside of the definition region. Afterwards the PSO compares the objective value of $\xi$ with the objective value of $z$ and saves the position with the lowest objective value as global best position. Therefore the PSO satisfies the *algorithm condition*. 

The next condition focuses on the behavior of the convergence of the PSO, which must ensure that a fixed convergence point exists. In [@FbEn2010], results were referenced from [@RiPo2007], which analyzed the convergence property of a deterministic PSO, i.e., when the uniformly random numbers are replaced by their upper bounds. Afterwards, the results were verified by simulations of the stochastic case. This work was subsequently extended by [@Mjly2006] to directly address the stochastic version of the standard PSO. 

In [@Mjly2006] three properties are described for the behavior of the PSO. First, the position of each particle $d$ is described as a sequence of random variables $\pmb{X}^d_i$ in iteration $i$. The particle $d$ converges to a fixed point if the expected sequence of positions in mean square converges to a point $p^d$ (property 1) and the variance of the sequence of positions tends to zero (property 2), which can be summarized as $\lim_{i\rightarrow \infty} E[\pmb{X}^d_i-p^d]^2=0$. And the entire particle swarm converges to a fixed point (property 3) if all sequences of particle positions converge to a single point $\hat{p}$, which can be summarized as $\lim_{i\rightarrow \infty} E[\pmb{X}_i^d-\hat{p}]^2=0$ $\forall d$. 

The properties 1-3 are dependent on the choices of $c_p$, $c_g$ and $w$, which can be found in [@Mjly2006]. The proofs and deeper explanation of the constraints for parameters are beyond the scope of this thesis. In the following the regions for the respective properties are visualized and the cyan region is further on called the convergent parameter region:


```{r convergenceregion, echo=F, fig.cap="Relationship between $w$ and $c$ when $c_p$=$c_g$=$c$ to distinguish between different convergence behaviors. The black area shows the convergence condition for the expected positions of each particle. The cyan area shows the convergence condition for the expected positions and the variances tend to zero for each particle. And the blue area shows the convergence condition for the expected positions, with the variances tending to zero for each particle and the entire swarm converging to a single point. This is a replicated representation made in [M. Jiang, 2006]."}
...
plot_ly(z = grid*1, type = "heatmap", x = colnames(grid), y=rownames(grid), colors = colorRamp(c("white", "black", "blue", "cyan")), showscale = FALSE) %>% 
...
$$
with a probability measure $\mu_i$, a distance measure $\text{dist}()$ and a optimality region $R_{\epsilon}$ defined as
$$
 R_{\epsilon} = \{z\in S \ | f(z) < \psi + \epsilon\}
$$
where $\psi$ denotes the essential infimum of $f$ on $S$, and $\epsilon > 0$.
...
::: {.theorem name="Local Search Alogirthm"}
Assume that $f$ is a measurable function, $S$ is a measurable subset of $\mathbb{R}^n$ and both the algorithm and the convergence condition for local search are satisfied. Let ${x_t}^{\infty}_{t=0}$ be a sequence generated by the algorithm, D. Then,
$$
