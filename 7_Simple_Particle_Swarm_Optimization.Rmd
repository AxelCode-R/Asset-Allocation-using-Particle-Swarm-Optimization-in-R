...
**Algorithm condition**: The mapping $D: S \times \mathbb{R}^n\rightarrow S$ should satisfy $f(D(x, \xi)) \leq f(x)$ and if $\xi \in S$, then $f(D(x, \xi)) \leq f(\xi)$.

That is, the *algorithm condition* is satisfied if the algorithm has a mapping $D$ that compares two positions and returns the position with the lowest objective value that is also in the definition region. In relation to the PSO, this concentrates on the behavior of changing the global best particle $x_{global}$. If a new local best particle $x_{new}$ is found, it is guaranteed that it is in the definition region, because all positions calculated are pushed back to there boundary, if they are outside of the definition region. Afterwards the PSO compares the objective value of $x_{new}$ with the objective value of $x_{global}$ and saves the position with the lowest objective value as global best particle. Therefore the PSO satisfies the *algorithm condition*. 

The next condition focuses on the point of convergence, which first



```{r convergenceregion, echo=F, fig.cap="Relationship between $\\omega$ and $c$ when $c_l$=$c_g$=$c$ to distinguish between different convergence behaviors. The black area shows the convergence condition for the expected positions of each particle. The cyan area shows the convergence condition for the expected positions and the variances tend to zero for each particle. And the blue area shows the convergence condition for the expected positions, with the variances tending to zero for each particle and the entire swarm converging to a single point. This is a replicated representation made in [@Mjly2006]."}

# Relationship between $$\\omega$$ and $$c$$ when $$c_l$$=$c_g$$=$c$$ to distinguish between different convergence behaviors. The black area shows the convergence condition for the expected positions of each particle. The cyan area shows the convergence condition for the expected positions and the variances tend to zero for each particle. And the blue area shows the convergence condition for the expected positions, with the variances tending to zero for each particle and the entire swarm converging to a single point. This is a replicated representation made in (ref:Mjly2006)
...
the PSO compares it to the $x_{global}$ and if its objective values 



Starting with the convergence to a local minimum, the PSO converges to a local minimum if the algorithm condition and the convergence condition for local search is satisfied.
...
Starting with the congerence into a local minimum, the PSO has to satisfie the following local search condition for algorithms:
