...
itp_QP <- function(pool_returns, bm_returns){
  ...
  mat <- list(
    ...
    bvec = c(
      ...
      rep(-0.04, ncol(pool_returns)) # w <= 0.05
    ),
    meq = 0
  )
  ...
  res <- list(
    "value" = as.numeric(0.5 * t(qp$solution) %*% mat$Dmat %*% qp$solution - t(mat$dvec) %*% qp$solution),
    "var" = as.numeric(
  ...
  time_it <- system.time({
    pso_res <- pso(
      ...
      fn = function(x){
        fitness <- calc_fit(x)
        constraints <- calc_const(x)
        return(fitness+1000*constraints)
      },
      lower = 0,
      upper = 0.04,
      control = list(
