...
This section summarizes the mathematical analysis of the convergence of the standard PSO performed in [@FbEn2010], which has the main goal to prove whether the PSO converges to a local or even a global minimum. This section provides only a simplified and therefore inaccurate reproduction of all relevant results of the source, so it is recommended to also look at the exact statements and proofs in the source. At first the PSO has to satisfy the algorithm condition for random search algorithms and the objective function $f$: \newline

**Algorithm condition:** The mapping $D: S \times \mathbf{R}^n\rightarrow S$ should satisfy $f(D(x, \xi)) \leq f(x)$ and if $\xi \in S$, then $f(D(x, \xi)) \leq f(\xi)$.

That is, the *algorithm condition* is satisfied if the algorithm has a mapping $D$ that compares two positions and returns the position with the lowest objective value that is also in the definition region. In relation to the PSO, this concentrates on the behavior of changing the global best particle $x_{global}$. If a new local best particle $x_{new}$ is found, it is garanteed that it is in the definition region, because all positions caluclated are pushed back if they are outside of the definition region. Afterwards the PSO compares the objective value of the new local best particle $x_{new}$ with the objective value of the $x_{global}$ and saves the lowest as global best particle. Therefore the PSO satisfies the *algorithm condition*. 
...
::: {.theorem #localsearch name="Local Search Alogirthm"}
Assume that $f$ is a measurable function, $S$ is a measurable subset of $\mathbf{R}^n$ and both the algorithm and the convergence condition for local search are satisfied. Let ${x_t}^{\infty}_{t=0}$ be a sequence generated by the algorithm, D. Then,
$$
